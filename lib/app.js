...
App.bootstrap = function (app, root) {
  ...
  app.paths = {
    preboots: typeof app.given.preboots === 'string' ? app.given.preboots : path.join(root, 'lib', 'preboots'),
    middlewares: typeof app.given.middlewares === 'string' ? app.given.middlewares : path.join(root, 'lib', 'middlewares'),
    routes: typeof app.given.routes === 'string' ? app.given.routes : path.join(root, 'lib', 'routes')
  };
  ...
  app.before('setup', function (instance, options, done) {
    var preboots = typeof app.given.preboots === 'function' ? app.given.preboots : require(app.paths.preboots);

    preboots(app, options, function (err) {
      if (err) {
        return done(err);
      }

      var middlewares = typeof app.given.middlewares === 'function' ? app.given.middlewares : require(app.paths.middlewares);
      var routes = typeof app.given.routes === 'function' ? app.given.routes : require(app.paths.routes);

      // Call the preboots for `./lib/middlewares` and `./lib/routes`
      app.preboot(middlewares);
      app.preboot(routes);
